...
import serverside.ConsonantProcessor;
import serverside.PunctuationProcessor;
import serverside.SentenceProcessor;
import serverside.VowelProcessor;
	...
	public static void main(String[] args) {
        ...
        try {
        	...
        	while (true) {
				...
				VowelProcessor vwprocessor = new VowelProcessor(receivedData);
				ConsonantProcessor conprocessor = new ConsonantProcessor(receivedData);
				PunctuationProcessor puncprocessor = new PunctuationProcessor (receivedData);
	            ...
	            int totalVowels = vwprocessor.countvowel();
	            byte[] outDataVowel = vwprocessor.convertToByteArray(totalVowels);
	            int totalConsonant = conprocessor.countconsonant();
	            byte[] outDataConsonant = conprocessor.convertToByteArray(totalConsonant);
	            int totalPunctuation = puncprocessor.countpunctuation();
	            byte[] outDataPunctuation = puncprocessor.convertToByteArray(totalPunctuation);
	            ...
	            int sizeOutConsonant = outDataConsonant.length;
	            int sizeOutPunctuation = outDataPunctuation.length;
